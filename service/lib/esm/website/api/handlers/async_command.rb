...
module ESM
  module Website
    class API
      module Handlers
        ##
        # Website API handler that dispatches a persisted {ESM::ServiceCommand} into the command
        ...
        class AsyncCommand
          def self.call(command_id:)
            command = ESM::ServiceCommand.includes(:user, :server, :community).find_by(id: command_id)
            raise ArgumentError, "Unknown command: #{command_id}" if command.nil?

            Concurrent::Promise.execute do
              ESM::Database.with_connection do
                command.command_class.website_async_hook(command)
              end
            end
          end
        end
