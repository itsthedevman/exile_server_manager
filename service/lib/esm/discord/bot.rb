  ...
  module Discord
    class Bot < Discordrb::Bot
      ...
      def initialize
        ...
        @waiting_for = {}
        @mutex = Mutex.new
        @delivery_overseer = ESM::Discord::Bot::DeliveryOverseer.new

        super(token: ESM.config.token, intents: INTENTS)
      end

      ##
      # Loads ESM commands and connects to Discord.
      #
      ...
      def esm_ready(_event)
        ...
        if ready?
          warn!("ESM::Discord::Bot#esm_ready called after bot was ready")
          return
        end
        ...
        info!(
          status: @esm_status,
          invite_url: invite_url(permission_bits: ESM::Discord::Bot::PERMISSION_BITS),
      ...
      def esm_member_join(event)
        ...
        ESM::Discord::Event::MemberJoin.new(event).run!
      end
      ...
      def log_error(error_hash)
        ...
        ESM.discord_bot.deliver("```#{error_message}```", to: ESM.config.error_logging_channel_id)
      end
