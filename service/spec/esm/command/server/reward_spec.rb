    ...
    describe "#on_execute", requires_connection: true do
      ...
      context "when choosing a reward package by id" do
        ...
        it "honors the package on a server too old for vehicles" do
          # Written here rather than asserted. A connected server reports whatever extension happens to be built, and
          # this is about the version rather than about the build sitting in the tree.
      ...
      # A corpse and an empty seat answer the same way. Exile finds a player by walking the living, so by the time the
      # command asks there is nothing left to tell the two apart with, and one message covers both.
      context "when the player is not alive on the server" do
        ...
        it "is expected to raise AlivePlayer" do
          ...
          embed = latest_message
          expect(embed.description).to match("in game and alive")
        end
      end
