...
export default class extends ApplicationController {
  ...
  // Back-navigation reconnects the controller against Turbo's restored DOM, which would otherwise snap the reader back to
  // page one of the unsorted list. Rehydrating from sessionStorage lands them on the same sort, search, and page.
  restoreState() {
    this.page = 1;
    this.sortKey = null;
  ...
  // Any change to what's being matched sends the reader back to the first page, since the page they were on may no longer
  // exist under the new search.
  filter() {
    this.page = 1;
    this.render();
  }

  // Clicking the active column flips its direction; a fresh one adopts its natural first order - A to Z for text,
  // highest-first for numeric columns.
  sort(event) {
  ...
  render() {
    ...
    // Re-seat this page's rows in sorted order, then hide everything not on the page. Appending a node that already lives
    // in the tbody just moves it, so this reorders in place without rebuilding the table.
  ...
  matchingRows() {
    ...
    return this.rowTargets.filter((row) => query === "" || row.dataset.search.includes(query));
  }

  // Sorts a copy so the original target order stays the fallback the rows return to once no column is chosen. Array sort
  // is stable, so that order also breaks ties within equal values.
  sortRows(rows) {
  ...
  // data-sort-key="level" addresses the row's data-sort-level, which reaches the element as dataset.sortLevel.
  sortAttribute() {
  ...
  saveState() {
    ...
    try {
      ...
      // sessionStorage can be full or blocked (private mode); persistence is a nicety, so a failure just means the next
      // back-navigation falls back to the default view.
    }
  }
