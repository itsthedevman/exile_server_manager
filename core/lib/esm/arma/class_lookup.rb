    ...
    class ClassLookup
      ...
      class << self
        ...
        def cache
          ...
          # All classes are stored as YAML files in core/config/arma_classes.
          # Anchor the path to this file's location so the lookup works no matter
          # which app (service, website, specs) is the current working directory.
          class_data_dir = File.expand_path("../../../config/arma_classes", __dir__)

          # Convert the YAML file into a lookup table where the key is the class name and the value is an instance of CLASS_ENTRY
          Dir["#{class_data_dir}/*.yml"].each do |file_path|
