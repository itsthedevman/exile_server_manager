    ...
    module Territory
      class ServerTerritories < ApplicationCommand
        ...
        def on_execute
          ...
          @territories = query_exile_database!("all_territories").map(&:to_datum)
        ...
        def check_for_no_territories!
          raise_error!(:no_server_territories, user: current_user, server_id: target_server.server_id) if @territories.blank?
        end
      end
    end
  end
