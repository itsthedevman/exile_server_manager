...
//!
//! The verification public key is DER-encoded SubjectPublicKeyInfo and is baked
...
/// using the public key baked in at compile time.
///
/// The key is stored as DER-encoded SubjectPublicKeyInfo (44 bytes).
...
///
/// Normally this resolves to the key baked in at compile time. Under the `testing` feature a process may install
/// a different one, which is the only way the suite can exercise what happens *after* a signature verifies: with the
/// baked-in key unreachable, every manifest a test can build is unsigned as far as this crate is concerned, and
...
pub mod test_key {
    ...
    /// Go back to the key baked in at compile time.
    pub fn clear() {
        if let Ok(mut slot) = OVERRIDE.write() {
            *slot = None;
        }
...
///
/// Intended for test use only — do not call from non-test paths.
